...
You have access to `gh`, `git`, and other CLI tools. Use `gh help` if you need 
to look something up.

Start by checking your current Git branch. If needed, create a new branch from 
`bitnet`, not `main`. Then create a draft pull request tied to issue #TASK# 
using:
...
* Save and commit often with small meaningful messages. Keep commits small, clear, and focused.
...
Go through the comments and **fix every issue that hasn't already been 
...
Do not leave files uncommitted or untracked. Keep commits small, clear, and 
focused.
...
modify or rewrite it. However, benchmark numbers included in the output are 
real and must be preserved as-is.

Finally, push your branch. **Your working directory must be clean. All changes 
must be committed and pushed.** Get the PR ready fast, with zero noise, zero 
surprises, and no extra work for anyone -- especially you.
