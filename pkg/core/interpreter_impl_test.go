...
func TestHandleExecResult(t *testing.T) {
	tests := []struct {
		name       string
		source     string
		execResult *primitive.ExecResult
		want       interface{}
		wantErr    bool
	}{
		{
			...
			source: "test",
			execResult: primitive.NewExecResult(
				[]*parsers.Instruction{},
				[]interface{}{},
			),
			want:    []interface{}{},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			i := NewInterpreter("test", map[string]string{
				"return": "/gnd/return",
				"error":  "/gnd/error",
			})
			got, err := i.HandleExecResult(tt.source, tt.execResult)
			if (err != nil) != tt.wantErr {
				t.Errorf("HandleExecResult() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("HandleExecResult() = %v, want %v", got, tt.want)
			}
		})
	}
}
