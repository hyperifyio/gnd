...
func TestTensorOps(t *testing.T) {
	...
	ops, err := NewTensorOps(10, 8) // maxSeqLength=10, hiddenSize=8
	...
	t.Run("ReshapeAndCopy", func(t *testing.T) {
		...
		tensor, err := ops.ReshapeAndCopy(data, 1, 5, 8)
		require.NoError(t, err)
	...
	t.Run("GetLastHiddenState", func(t *testing.T) {
		...
		lastHiddenState, err := ops.GetLastHiddenState(tensor, 5, 8)
		require.NoError(t, err)
