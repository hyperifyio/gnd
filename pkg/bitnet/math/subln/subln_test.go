...
func TestNewSubLN(t *testing.T) {
	...
	subln, err := NewSubLN(hiddenSize, epsilon)
...
func TestSubLNNormalize(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if len(tt.input) == 0 {
				subln, _ := NewSubLN(1, tt.epsilon) // hiddenSize doesn't matter for empty input
				got, err := subln.Normalize(tt.input)
				require.NoError(t, err)
				tt.checkFunc(t, got, tt.expected)
				return
			}
			subln, _ := NewSubLN(len(tt.input[0]), tt.epsilon)
			got, err := subln.Normalize(tt.input)
			require.NoError(t, err)
			tt.checkFunc(t, got, tt.expected)
		})
...
func TestSubLNGamma(t *testing.T) {
	hiddenSize := 4
	subln, _ := NewSubLN(hiddenSize, 1e-5)
	...
	// Test getting gamma
	gamma, err := subln.GetGamma()
	require.NoError(t, err)
	if len(gamma) != len(newGamma) {
		t.Errorf("expected gamma length %d, got %d", len(newGamma), len(gamma))
	}
	for i, g := range gamma {
		if g != newGamma[i] {
