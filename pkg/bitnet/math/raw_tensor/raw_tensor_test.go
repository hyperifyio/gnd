...
func TestNewRawTensorFrom(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("input.Shape() failed: %v", err)
			}
			data, err := input.Data()
			if err != nil {
			...
			rt, err := NewRawTensorFromData(inputShape, data)
			if err != nil {
	...
	tests := []struct {
	...
	}{
		{
			...
			fn: func(t *testing.T) error {
				...
				_, err = NewRawTensorFromData(shape, data)
				return err
			},
			wantErr: true,
		},
		{
			name: "3D tensor",
			fn: func(t *testing.T) error {
				tensor, err := tensor.NewTensor(2, 2, 2)
				if err != nil {
					return err
				}
				shape, err := tensor.Shape()
...
func BenchmarkRawTensorFrom(b *testing.B) {
	...
	for _, size := range sizes {
		b.Run("", func(b *testing.B) {
			// Create input tensor
			input, err := tensor.NewTensor(size.rows, size.cols)
			if err != nil {
				b.Fatalf("NewTensor(%d, %d) failed: %v", size.rows, size.cols, err)
			}
			shape, err := input.Shape()
			if err != nil {
				b.Fatalf("input.Shape() failed: %v", err)
			}
			data, err := input.Data()
			if err != nil {
				b.Fatalf("input.Data() failed: %v", err)
			}
			...
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				_, _ = NewRawTensorFromData(shape, data)
			}
		})
	}
}
