...
func TestNewRoPE(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			rope, err := NewRoPE(tt.base, tt.maxSeqLen, tt.dim)
			if tt.shouldPanic {
				require.Error(t, err)
				return
			}
...
func TestApplyRoPE(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := rope.ApplyRoPE(tt.vector, tt.position)
			if tt.shouldPanic {
				require.Error(t, err)
				return
			}
			...
			// Check dimensions
			if len(got) != tt.dim {
				t.Errorf("expected result length %d, got %d", tt.dim, len(got))
			}
...
func TestApplyRoPEBatch(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := rope.ApplyRoPEBatch(tt.vectors, tt.startPos)
			if tt.shouldPanic {
				require.Error(t, err)
				return
			}
			...
			// Check batch size
			if len(got) != len(tt.vectors) {
				t.Errorf("expected %d results, got %d", len(tt.vectors), len(got))
			}

			// Check each vector in the batch
			for i, vector := range tt.vectors {
				expected, err := rope.ApplyRoPE(vector, tt.startPos+i)
				require.NoError(t, err)
				for j := 0; j < tt.dim; j++ {
					if math.Abs(float64(got[i][j]-expected[j])) > 1e-5 {
						t.Errorf("vector %d, dimension %d: expected %f, got %f", i, j, expected[j], got[i][j])
					}
				}
			}
		})
...
func BenchmarkApplyRoPE(b *testing.B) {
	...
	rope, err := NewRoPE(base, maxSeqLen, dim)
