...
func TestNewLMHead(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			layer, err := NewLMHead(tt.hiddenDim, tt.vocabSize)
			if tt.wantPanic {
				require.Error(t, err)
				return
			}
	...
	tests := []struct {
	...
	}{
		{
			...
			input: func() *tensor.Tensor {
				t, err := tensor.NewTensor(2, 3, 512)
				if err != nil {
			...
			weights: func() *tensor.Tensor {
				t, err := tensor.NewTensor(32000, 512)
				if err != nil {
					panic(err) // This is a test setup, so we can panic
				}
				for i := 0; i < 32000; i++ {
		...
		},
		{
			...
			vocabSize: 32000,
			input: func() *tensor.Tensor {
				t, err := tensor.NewTensor(2, 3, 256)
				if err != nil {
					panic(err) // This is a test setup, so we can panic
				}
				for i := 0; i < 2; i++ {
...
func TestLMHead_SetWeights(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			vocabSize: 128000,
			weights: func() *tensor.Tensor {
				t, err := tensor.NewTensor(128000, 2560)
				if err != nil {
					panic(err) // This is a test setup, so we can panic
				}
				for i := 0; i < 128000; i++ {
...
func TestLMHead_GetWeights(t *testing.T) {
	layer, err := NewLMHead(2560, 128000)
	require.NoError(t, err)
	require.NotNil(t, layer)
	...
	// Set weights
	weights, err = tensor.NewTensor(128000, 2560)
	if err != nil {
...
func BenchmarkLMHead_Forward(b *testing.B) {
	...
	// Create input tensor
	input, err := tensor.NewTensor(32, 16, 2560)
	if err != nil {
